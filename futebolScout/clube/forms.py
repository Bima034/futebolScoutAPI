...
class ClubeCreaterForm(forms.ModelForm):
    ...
    fundacao = forms.DateField(label='Data de fundação', widget=forms.DateInput(attrs={'type': 'date'}, format='%Y-%m-%d'))

    
    class Meta:
        model = Clube
        fields = ['nome', 'sigla', 'descricao', 'pais', 'federacoes', 'logo_path', 'treinador', 'presidente', 'estadio', 'fundacao']
